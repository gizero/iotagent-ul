...
function generateTopics(callback) {
    ...
    topics.push('/+/+/' + constants.MEASURES_SUFFIX + '/+');
    topics.push('/+/+/' + constants.MEASURES_SUFFIX);
    topics.push('/+/+/' + constants.CONFIGURATION_COMMAND_SUFFIX);
...
function multipleMeasures(apiKey, device, message) {
    ...
    try {
        ...
        logger.error(context, 'Parse error parsing incoming MQTT message [%]', messageStr);
        return;
    }
...
function mqttMessageHandler(topic, message) {
    ...
    function processMessageForDevice(device, apiKey, topicInformation) {
        if (topicInformation[4]) {
            ...
            if (topicInformation[3] === constants.CONFIGURATION_COMMAND_SUFFIX) {
                updateCommand(apiKey, device, message.toString());
            } else if (topicInformation[3] === constants.MEASURES_SUFFIX) {
...
function start(callback) {
    ...
    mqttClient.on('error', function(e) {
        logger.fatal('Couldn\'t connect with MQTT broker: %j', e);
        callback(e);
    });
